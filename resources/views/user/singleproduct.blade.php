        ...
        <div class="container">
            ...
            <div class="infoproduct">
                <div class="background">
                    <img style="min-width: 330px;max-width: 330px;min-height: 330px;max-height: 330px;" src="{{asset($product->imagepath)}}" alt="">
                </div>
                <div class="details">
                    <h1 style="font-weight: bold">{{$product->name}}</h1>
                    <p>{{$product->description}}</p>
                    <h2><span style="color: #e99c2e;">Price</span> {{$product->price}} $</h2>
                    <h2>Available Quantity - {{$product->qty}}</h2>

                    <form action="{{route('addtocart',['id'=>$product->id])}}" method="post">
                        @csrf
                        <input type="hidden" name="price" value="{{$product->price}}">
                        <label for="">Product Quantity</label>
                        <input type="number" name="qty" class="form-control" min="1" max="{{$product->qty}}" placeholder="1">
